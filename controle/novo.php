...
$query = mysql_query("select * from item where item_pk = $item_pk");
    ...
    if (mysql_num_rows($query) > 0) {
		 header("location: ../novo.php?item_pk=$item_pk&titulo=Ops! Não foi possível concluir.&mensagem=Já existe um produto cadastrado com esse número de patrimônio.&class=alert alert-error&type=Erro!");
	}
	else{
	...
	echo "categoria: ". $_GET['categoria'];
		...
		if (!$query) {
			header("location: ../novo.php?item_pk=$item_pk&titulo=Ops! Não foi possível concluir.mensagem=Ocorreu algum erro na inserção dos dados.&class=alert alert-error&type=Erro!");
		}
		
		else{
			header("location: ../novo.php?item_pk=$item_pk&titulo=Tudo certo.&mensagem=Item inserido com sucesso!&class=alert alert-success&type=Tudo certo.");
		}
	}
?>
